...
//	cout << "build suffix array index\n";
	...
	int N = sq.size();
	cout << "weighted special string length:" << N/1000000 << " MB\n";
#if 1 
	int * SA	= new int [N];
	int * LCP	= new int [N];
	int * ME	= new int [N];
	...
	{//print & check
		...
		for ( int i = 0; i < N; i++ )
		{
			cout << i << '\t' << SA[i] << '\t' << sq.substr(SA[i], ME[SA[i]]) << '\t' << LCP[i] << endl;
		}
	}

	end = get_time::now();
	auto diff2 = end - begin;
	cout<<"Indexing time is:  "<< chrono::duration_cast<chrono::milliseconds>(diff2).count()<<" ms "<<endl;
#if 0
